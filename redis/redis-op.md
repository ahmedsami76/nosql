...
- [Part IV — The Redis object hierarchy](#part-iv--the-redis-object-hierarchy)
  - [IV.0 Orientation — the model you are carrying, and the one you need](#iv0-orientation--the-model-you-are-carrying-and-the-one-you-need)
  - [IV.1 The whole hierarchy on one page](#iv1-the-whole-hierarchy-on-one-page)
  - [IV.2 Level 1 — the deployment](#iv2-level-1--the-deployment)
  - [IV.3 Level 2 — the server instance](#iv3-level-2--the-server-instance)
  - [IV.4 Level 3 — the logical database](#iv4-level-3--the-logical-database)
  - [IV.5 Level 4 — the keyspace](#iv5-level-4--the-keyspace)
  - [IV.6 The missing level — where "tables" went](#iv6-the-missing-level--where-tables-went)
  - [IV.7 Level 5 — the key](#iv7-level-5--the-key)
  - [IV.8 Level 6 — the value object and its encoding](#iv8-level-6--the-value-object-and-its-encoding)
  - [IV.9 Level 7 — elements inside a value](#iv9-level-7--elements-inside-a-value)
  - [IV.10 What is *not* in the hierarchy](#iv10-what-is-not-in-the-hierarchy)
  - [IV.11 Relational vocabulary → Redis vocabulary](#iv11-relational-vocabulary--redis-vocabulary)
  - [IV.12 The rules that fall out of the hierarchy](#iv12-the-rules-that-fall-out-of-the-hierarchy)
- [Part V — Client access: connect, validate, administer, author](#part-v--client-access-connect-validate-administer-author)
  - [V.0 The client landscape](#v0-the-client-landscape)
  - [V.1 Client 1 — `redis-cli` via `kubectl exec`](#v1-client-1--redis-cli-via-kubectl-exec)
  - [V.2 Client 2 — RedisInsight](#v2-client-2--redisinsight)
  - [V.3 Client 3 — Python in JupyterLab](#v3-client-3--python-in-jupyterlab)
  - [V.4 Capability matrix — which client for which job](#v4-capability-matrix--which-client-for-which-job)
- [Part VI — Basic Redis operations](#part-vi--basic-redis-operations)
  ...
  - [VI.2 Keyspace design in practice](#vi2-keyspace-design-in-practice)
...
- [Instructor's quick-reference: key takeaways](#instructors-quick-reference-key-takeaways)
...
**The teaching arc.** Part I and Part II are conceptual — what a key-value store is and what Redis specifically is. Part III stands the lab up. Part IV is the mental model everything else depends on: the object hierarchy, from deployment down to the individual element inside a value, and — just as importantly — the levels SQL has that Redis does not. Part V is the one students actually keep: three different clients, each taken through the same four verbs — **connect, validate, administer, author**. Parts VI and VII are hands-on operations against a realistic dataset.

**Why the object model comes before the clients.** It is tempting to connect first and start typing `SET` and `GET`. Resist it. A `SET` you do not have a model for is a magic incantation; a `SET` you understand is "write a key into the keyspace of one logical database, creating a value object whose type is now permanent." Part IV pays for itself the moment you start authoring in Part V.

> **Instructor note:** if you are recording this, there are two natural chapter breaks. The first is before Part IV — everything prior is "get the environment up." The second is before Part V, where the model gives way to hands-on work. Part IV is the chapter students will re-watch.
...
data:
  redis.conf: |
    ...
    # live with OBJECT ENCODING before and after crossing a threshold (IV.8).
...
> **Minimum Redis version for this config.** `set-max-listpack-entries` requires **Redis 7.2 or newer** — on 7.0 the server refuses to start with `Bad directive or wrong number of arguments`. The `redis:8-alpine` image we pin satisfies this. If you retarget the lab at an older major, drop that one line. (The hash-field TTL commands in IV.7 need 7.4+.)
...
#    This is the address you will type into RedisInsight in V.2.
# -----------------------------------------------------------------------------
apiVersion: v1
kind: Service
metadata:
...
> **Note the asymmetry worth pointing out:** Jupyter receives `REDIS_PASSWORD` from the Secret as an environment variable, but RedisInsight does not — RedisInsight stores credentials in its own encrypted local database when you add a connection through the UI. That is why V.2 has you type the password in by hand.
...
## Part IV — The Redis object hierarchy
...
### IV.0 Orientation — the model you are carrying, and the one you need

Before a single command, get the shape of the thing straight. Almost every Redis mistake a SQL practitioner makes traces back to importing a container hierarchy that does not exist here.

**The model you arrived with:**

```text
SERVER
└── DATABASE          named, created with DDL, a real isolation boundary
    └── SCHEMA        a namespace you can grant on
        └── TABLE     a declared object with a name and a shape
            ├── COLUMN  typed, constrained, identical for every row
            └── ROW     one record, addressable by primary key
```

Five levels of *declared structure* above the data. The engine knows what a `customers` table is, knows that `tier` is a `varchar(10)`, and will refuse writes that violate either. You describe your data once, in DDL, and the engine enforces that description forever.

**The model Redis actually has:**

```text
SERVER INSTANCE
└── DATABASE 0..15    a NUMBER. No name, no DDL, no isolation. Always exists.
    └── KEYSPACE      one flat hash table. Nothing between it and your keys.
        └── KEY       a byte string you invented. The only address that exists.
            └── VALUE one of ~10 data structures, each with its own inner shape
```

Four levels, and only one of them is something you create. There is no table. There is no schema. There is no column. **The engine has no idea what a "customer" is**, and it never will.

#### The three consequences

**1. The structure moved from above the record to inside it.**

SQL spreads a customer across a table definition (columns), a row (values), and constraints (rules). Redis has none of those places to put it, so it all goes into the value:

```text
SQL                                  REDIS
───                                  ─────
CREATE TABLE customers (             (nothing — no DDL exists)
  id     varchar(6) PRIMARY KEY,     -> the key name: "retail:customer:C00001"
  name   varchar(50),                -> a field inside a Hash
  tier   varchar(10),                -> another field inside the same Hash
  points int                         -> another field
);                                   
INSERT INTO customers VALUES (...);  -> HSET retail:customer:C00001 name ... tier ... points ...
```

The Hash *is* the row. The key name *is* the primary key. And the "table" — the thing that groups all customers together — has no representation in the engine at all. It exists only as a shared prefix in the key names, which is IV.6.

**2. The engine validates nothing.**

There is no `NOT NULL`, no `varchar(10)`, no foreign key, no `CHECK`. You can write `tier` as `"gold"` on one key and `42` on the next, and Redis will store both without complaint. Everything a schema would have guaranteed is now your application's job. That is the price of the speed — no validation on the write path means nothing to check before the write lands.

**3. Grouping is a convention, so *you* must invent and enforce it.**

Because there is no table object, there is no `SELECT * FROM customers` and no way to ask "how many customers are there?" without either scanning the whole keyspace or maintaining a counter yourself. What replaces the table is a naming discipline — and unlike DDL, nothing enforces it. A typo in a key name does not raise an error; it silently creates a second, orphaned key that no code will ever read.

#### The trade you are making

| | Relational | Redis |
|---|---|---|
| Where structure is declared | Up front, in DDL | Nowhere — it lives in your code |
| Who enforces it | The engine, on every write | You, by convention and review |
| Cost of a bad model | Caught at write time, loudly | Discovered in production, silently |
| Cost of a good model | Migrations, planning, rigidity | A key-builder module and discipline |
| What you get for it | Correctness guarantees, ad-hoc query | Microsecond point lookups, trivial scale-out |

This is not "Redis is schemaless." Redis is **schema-on-read with the schema in your application**. The schema absolutely exists — it is just written in Python instead of DDL, and enforced by your code review instead of the engine. Sections IV.1 through IV.12 map out exactly where each piece of it goes.

#### How to follow along

Everything in this part is exploration, not authoring, so `redis-cli` inside the pod is enough — that path was established in III.8 and needs no setup:

```bash
kubectl -n nosql-lab exec -it redis-0 -- redis-cli
```

Python snippets appear where the point is easier to see in code. They are self-contained: IV.4 builds the client objects (`retail`, `sessions`) that the later cells reuse. The full three-client treatment — including how to *write* data — is Part V, deliberately placed after this one.

---

### IV.1 The whole hierarchy on one page
        ...
        └── KEYSPACE             a hash table of key -> value, plus a parallel EXPIRES table
            └── KEY              binary-safe string · <= 512 MB · unique in this DB · one TYPE forever
                └── VALUE OBJECT type + encoding + TTL + LRU/LFU counter + refcount
                    └── ELEMENTS the contents, which differ per type (see IV.9)
```
...
### IV.3 Level 2 — the server instance
...
### IV.4 Level 3 — the logical database
...
5. **Pub/Sub ignores them entirely.** A message published from a connection on db 0 is delivered to a subscriber on db 9. Channels are instance-scoped (IV.10).
...
#
# Calling retail.select(3) mutates whichever pooled connection happens to be
...
The expires dict holds an entry only for keys that actually have a TTL, which is why `INFO keyspace` reports `keys` and `expires` as two separate numbers.
...
DBSIZE                       # keys in the current database — O(1), just a counter
RANDOMKEY                    # one random key — a cheap way to sample the shape
```

#### "Keyspace" means two things — disambiguate them early

The word gets used loosely, and the two senses matter in different conversations:

| Sense | Meaning | Where you meet it |
|---|---|---|
| **Precise** | The hash table belonging to *one* logical database | `INFO keyspace`, the internals, `dict` in the source |
| **Colloquial** | "all the keys on this instance", across every database | "scan the keyspace", "keyspace design", `--bigkeys` |

An instance therefore has 16 keyspaces in the precise sense, and people will still say "the keyspace" meaning all of them. This module uses the colloquial sense everywhere except right here.

#### What the hash table buys you, and what it costs

Every property in the table below is a direct consequence of "a keyspace is a hash table." This is the paragraph that explains most of Redis's behaviour:

| Because it is a hash table… | You get | You lose |
|---|---|---|
| Lookup is a hash probe | O(1) `GET` regardless of key count | — |
| There is no tree | Flat latency as data grows | Any ordering of key names |
| Keys are not sorted | — | Range scans over key names; there is no "next key" |
| No prefix index exists | — | `SCAN MATCH` filters *after* fetching — the glob does not narrow the walk |
| Sizing is power-of-two with chaining | Predictable memory growth | — |
| Resize is **incrementally rehashed** across `ht[0]`/`ht[1]` | Growing to millions of keys never stalls the server (II.3) | — |

The fourth row is the operationally important one and is routinely misread. `SCAN 0 MATCH retail:customer:* COUNT 100` does **not** jump to the customers. It walks 100-odd buckets of the whole table and then discards the keys that did not match. A prefix that covers 1% of your keys costs the same full traversal as `MATCH *` — you just receive fewer results. There is no index on key names, so a prefix is a filter, never a seek.

#### Per-key overhead — why key names are a capacity decision

A key is not free even when its value is tiny. Each entry costs the key string itself, plus the `robj` header, plus dict-entry and bucket overhead — on the order of **50–100 bytes before the value**, and one more entry in the expires dict if it has a TTL.

```redis
# See it for yourself: the value is 1 byte, the key costs far more.
SET a:1 x
MEMORY USAGE a:1

SET this:is:a:considerably:longer:key:name:for:comparison:1 x
MEMORY USAGE this:is:a:considerably:longer:key:name:for:comparison:1
```

At 100 million keys, trimming 20 bytes from your naming convention saves roughly 2 GB of RAM. That is a real number and it is why IV.6 tells you to keep names short — but *readable*, because unreadable keys cost more in engineer time than they ever save in memory.

#### The keyspace is per-database, and that has consequences

Every keyspace command operates on the **currently selected** database only, which follows directly from there being one dict per database:

```redis
SELECT 0
DBSIZE                       # counts db0's dict only
SCAN 0                       # walks db0's dict only
RANDOMKEY                    # samples db0's dict only
FLUSHDB                      # empties db0's dict only
KEYS *                       # (never do this) db0's dict only

# There is NO command that walks every database at once. To census an entire
# instance you loop the databases yourself:
#   for db in 0..15: SELECT db; DBSIZE
# INFO keyspace is the shortcut — it reports every non-empty database in one go.
INFO keyspace
```
...
### IV.6 The missing level — where "tables" went

Look at the hierarchy again and notice the gap:

```text
DATABASE 0
└── KEYSPACE
    └── ???                <- SQL has TABLE here. Redis has nothing.
        └── KEY -> VALUE
```

**There is no table.** No `CREATE TABLE`, no `ALTER`, no `DROP TABLE`, no `information_schema` to query, no object to inspect, no place the engine records that `retail:customer:C00001` and `retail:customer:C00002` have anything to do with each other. To Redis they are two unrelated byte strings that happen to share fourteen leading characters.

This is the level students keep reaching for and not finding, so it is worth being blunt: **the grouping you see is entirely in your head and in your key names.** Three things fill the gap the table left behind.

#### Filler 1 — the key name carries the hierarchy

Since the keyspace is flat, the only place to encode structure is the key string itself. The universal convention is colon-delimited segments:

```text
   retail  :  customer  :  C00001  :  orders
   ──────     ────────     ──────     ──────
   namespace  entity       identity   attribute
      │           │            │          │
      │           │            │          └─ which sub-collection of that record
      │           │            └─ the primary key value
      │           └─ what a relational model would have called the TABLE
      └─ the application / tenant — makes multi-tenancy and selective flush possible
```

Read against a relational model, one segment does the work of each missing level:

| Key segment | Relational analogue | Enforced by |
|---|---|---|
| `retail` | Database or schema | Convention |
| `customer` | Table name | Convention |
| `C00001` | Primary key value | Convention |
| `orders` | A related table / join | Convention |

Every one of those says **convention**. That is the whole point of this section. The colon is not a special character to Redis — it is a plain byte, exactly like `x` or `-`. Redis does not parse it, does not index on it, and would work identically if you used `|` or `/`. The convention matters because *everything around Redis* assumes it: RedisInsight builds its keyspace tree by splitting on `:`, `redis-cli --scan --pattern` is written against it, and every monitoring exporter groups metrics by prefix.

#### Filler 2 — the data structure choice carries the shape

A table declares its shape in columns. A Redis key declares its shape by *which of the ten types you picked* — and that decision is permanent (IV.7). This is why "choosing the data structure is the schema design":

```text
retail:customer:C00001            -> Hash    the customer record ("the row")
...
retail:order:O000123:items        -> List    line items, order preserved
retail:idx:customer:tier:gold     -> Set     a secondary index you maintain
retail:idx:product:price          -> ZSet    a sorted index: SKU -> price
retail:leaderboard:products       -> ZSet    SKU -> purchase count
retail:events:stream              -> Stream  the clickstream log
retail:metrics:views:2026-08-09   -> String  a counter, with a TTL
retail:hll:visitors:2026-08-09    -> HLL     approximate unique visitors
```

Eleven keys, eight different types, one conceptual "schema" — and not one line of DDL anywhere. Notice that several of those keys exist *only* to make queries possible that a relational engine would have answered from the table itself.

#### Filler 3 — you build the indexes the table would have given you

This is the consequence with teeth. In SQL, `CREATE TABLE customers` gets you a primary key index for free, and `CREATE INDEX` gets you any other access path with one statement — after which the engine keeps it correct forever.

In Redis the key name is the *only* index that exists. Any other way of finding data is a second data structure that **you write, you update, and you keep in sync**:

```text
The question                          What answers it        Who maintains it
─────────────                         ──────────────         ────────────────
"Give me customer C00001"             the key itself         Redis (free)
"All gold-tier customers"             a Set  retail:idx:customer:tier:gold      YOU
"Products between $100 and $500"      a ZSet retail:idx:product:price           YOU
"Orders placed in January"            a ZSet scored by timestamp                YOU
"How many customers exist?"           a counter key, or a full SCAN             YOU
"Customers named Ava"                 nothing — you did not build it            —
```

That last row is the one to sit with. In SQL an unindexed query is *slow*. In Redis an unindexed query is **impossible** — there is no scan-and-filter the engine will do for you at any price. If you did not build the access path in advance, the data cannot be reached except by walking every key.

And because you own both sides, they can drift:

```redis
# The write that looks complete but is not:
HSET retail:customer:C00001 tier "platinum"
# The customer is now platinum. The index still says gold. Every query that
# reads the index now returns a wrong answer, silently, forever.
```

The fix is that an entity write and its index writes are **one atomic unit** — a `MULTI`/`EXEC` or a Lua script — never two separate calls. Drifted indexes are the single most common correctness bug in hand-rolled Redis models, and VI.3 shows the pattern that prevents them.

#### Two views of the same thing

The mental split worth drawing: what you *think* you have, versus what is actually stored.

```mermaid
flowchart LR
    subgraph LOGICAL["What you designed — the model in your head"]
        direction TB
        T1["'customers'<br/>500 records"]
        T2["'products'<br/>200 records"]
        T3["'orders'<br/>2000 records"]
        IX["'indexes'<br/>tier, price, date"]
    end
    subgraph PHYSICAL["What Redis stores — one flat keyspace"]
        direction TB
        F["retail:customer:C00001 → Hash<br/>retail:customer:C00002 → Hash<br/>retail:product:SKU-00042 → Hash<br/>retail:order:O000123 → Hash<br/>retail:idx:customer:tier:gold → Set<br/>retail:idx:product:price → ZSet<br/>… 5,000 more, all siblings, no grouping"]
    end
    LOGICAL -->|"exists only in your<br/>naming convention"| PHYSICAL
```

Redis sees the right-hand box. Only your code and your key-naming discipline turn it back into the left-hand one.

#### Checking your work

Because nothing is declared, nothing is validated — so verification is something you do deliberately:

```redis
# "Show me the customer table" is really a prefix scan.
SCAN 0 MATCH retail:customer:* COUNT 100

# "How many rows?" has no cheap answer. Either count the index Set you built...
SCARD retail:idx:customer:tier:gold

# ...or walk the prefix, which is O(N) over the WHOLE keyspace, not over
# "the table" — because there is no table to restrict the walk to.
# (redis-cli --scan --pattern 'retail:customer:*' | wc -l)

# "Does this key follow the convention?" Nothing will tell you. A typo like
# retail:custommer:C00001 creates a perfectly valid key that no code reads.
TYPE retail:customer:C00001
```

> **The discipline that replaces DDL:** build every key name in one module, never with scattered f-strings. VI.2 turns this into a concrete key-builder class — which is as close to a schema definition as a Redis codebase gets, and should be reviewed with the same seriousness as a migration.

---

### IV.7 Level 5 — the key
...
OBJECT ENCODING retail:customer:C00001  # listpack | hashtable  (IV.8)
...
### IV.8 Level 6 — the value object and its encoding
...
# ---------------------------------------------------------------------------
retail.delete("demo:encoding")
retail.hset("demo:encoding", mapping={f"f{i}": f"v{i}" for i in range(10)})
print(f"  10 fields -> {retail.object('encoding', 'demo:encoding'):<10} "
      f"{retail.memory_usage('demo:encoding'):>6} bytes")

retail.hset("demo:encoding", mapping={f"f{i}": f"v{i}" for i in range(200)})
print(f" 200 fields -> {retail.object('encoding', 'demo:encoding'):<10} "
      f"{retail.memory_usage('demo:encoding'):>6} bytes")
...
retail.hdel("demo:encoding", *[f"f{i}" for i in range(190)])
print(f"  10 fields -> {retail.object('encoding', 'demo:encoding'):<10} "
      f"{retail.memory_usage('demo:encoding'):>6} bytes  <- still a hashtable")
retail.delete("demo:encoding")
```
...
# ---------------------------------------------------------------------------
retail.set("demo:int",   "42")                       # parses as an integer
retail.set("demo:short", "hello")                    # <= 44 bytes
retail.set("demo:long",  "x" * 100)                  # > 44 bytes
for k in ("demo:int", "demo:short", "demo:long"):
    print(f"{k:<12} encoding={retail.object('encoding', k):<8} "
          f"refcount={retail.object('refcount', k)}")
retail.delete("demo:int", "demo:short", "demo:long")
```

**A refcount detour that is worth the two minutes**, because it shows the instance level (IV.3) reaching down and changing how a value is stored.
...
Object sharing is switched off the moment you configure `maxmemory` with an LRU or LFU policy, because every object then needs its own private access counter and a shared object cannot carry per-key metadata. So on this lab's instance you will see `refcount=1` everywhere — and that is correct, not a broken demo. Two lines in a ConfigMap silently changed the physical representation of every small integer in the database.
...
### IV.9 Level 7 — elements inside a value
...
### IV.10 What is *not* in the hierarchy
...
### IV.11 Relational vocabulary → Redis vocabulary
...
| Transaction | `MULTI`/`EXEC`, or a Lua script | No rollback, no isolation levels — see IV.12 |
...
### IV.12 The rules that fall out of the hierarchy
...
## Part V — Client access: connect, validate, administer, author

You have a running Redis and, from Part IV, a model of how it organizes what you put in it. This part is about *working* it — and it is the part students keep long after the course, because in a real job you will use all three of these clients, often in the same afternoon.

Everything you author below now has a place to live in that model: a `SET` writes a key into the keyspace of one logical database; an `HSET` builds a value object whose encoding Redis picks for you; a `ZADD` builds the secondary index Redis will never build on its own.
...
### V.1 Client 1 — `redis-cli` via `kubectl exec`
...
```bash
# -n 3  : start on logical database 3 instead of 0 (see IV.4)
...
# to run against a live server — unlike KEYS (see IV.5).
...
**Transactions in the CLI.** `MULTI` queues commands; `EXEC` runs them as one atomic unit. There is no rollback — see IV.12.
...
| **Database index selector** (top of Browser) | The `SELECT n` control. Switch to db 1, watch the key list empty out, switch back. The clearest demo of logical databases you will get — see IV.4. |
...
Three checks that map one-to-one onto the CLI validation in V.1.2:
...
**Workbench** is the administrative surface. It is a command editor with autocomplete, inline documentation, and multi-command execution — everything from V.1.3 works here, and the built-in docs make it the friendliest place to learn a command you have not used before.
...
| **Pub/Sub** | Subscribe to channels and publish test messages | Channels are server-scoped, *not* per-database (IV.10) |
...
> **The limit worth stating plainly:** RedisInsight cannot change your Kubernetes manifests. Anything you do in the UI is runtime-only, exactly like `CONFIG SET`, and it disappears the moment the pod restarts. Durable change goes through the ConfigMap and a rollout (V.1.3).
...
Do this once for each type on camera. It is the fastest way to make the type system concrete, and it sets up Part IV:
...
> **Verification exercise for students.** Create `retail:customer:C88888` in the RedisInsight UI, then read it back with `kubectl exec redis-0 -- redis-cli HGETALL retail:customer:C88888`, then read it a third time from Python in V.3. Three clients, one identical result. That is the moment RESP stops being an abstract detail and becomes obvious.

---

### V.3 Client 3 — Python in JupyterLab
...
#### V.3.0 The Python library landscape
...
r = redis.Redis(
    ...
    db=0,                        # logical database index (0-15) — see IV.4
...
Everything in V.1.3 is reachable from Python, and here it becomes *repeatable* — which is the whole reason to administer from code rather than a prompt.
...
# running 'kubectl rollout restart statefulset/redis' (V.1.3).
```
...
# ConfigMap or an aclfile — a pod restart erases this user (V.1.3).
analyst.close()
...
```python
# ---------------------------------------------------------------------------
# One key of every core type, so the type system is concrete before Part IV.
...
for t in ("string", "hash", "list", "set", "zset", "stream", "hll", "bitmap"):
    ...
    report.append({
        ...
        "ENCODING": r.object("encoding", k),  # the physical representation (IV.8)
...
```python
# ---------------------------------------------------------------------------
# Clean up everything this section created, so Part IV starts from a known
...
### V.4 Capability matrix — which client for which job
...
### VI.2 Keyspace design in practice

IV.6 established *why* there is no table level and what fills the gap. This section turns that into working practice: the naming rules worth enforcing in code review, the structure-selection table, and the key-builder module that is the closest thing a Redis codebase has to a schema definition.

The principle to carry in: *naming is schema design*, and it is the highest-leverage decision you will make.
...
> Encoding transitions are covered in IV.8 — run that demo here again if you skipped it, now that the keyspace holds real data and `MEMORY USAGE` has something interesting to say.
...
| `WRONGTYPE Operation against a key holding...` | A key's type is fixed on creation | `DEL` the key, then recreate it with the intended type (IV.7) |
...
| `OBJECT IDLETIME` → `An LFU maxmemory policy is selected` | LFU tracks frequency, not access time | Use `OBJECT FREQ` instead (IV.8) |
...
| A key expired "late" | Expiry is lazy plus probabilistic sampling | Expected behaviour — never depend on exact-moment expiry (IV.7) |
| Subscriber on db 9 receives a message published from db 0 | Pub/Sub channels are instance-scoped | Working as designed (IV.10) |
| `SELECT 1` → error | Redis Cluster supports only database 0 | Redesign around key prefixes (IV.4) |
| `CROSSSLOT Keys ... don't hash to the same slot` | Multi-key operation across slots in Cluster | Use hash tags: `retail:{C00001}:profile` (IV.2) |
